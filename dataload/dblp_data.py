...
from .walks2adj import CustomEdgeDataset
...
class DBLPDataset(DGLDataset):
    def __init__(self, name='my_dataset', slice_interval=1, time_span=60*60*24*365, start_timestamp=0, verbose=True, tf_decay_rate=0.1,
    ...
    def process(self):
        ...
        edges_data = pd.read_csv('./data/DBLP/dblp_reindex.txt', 
                                 header=None, sep=' ', names=['Src', 'Dst', 'Timestamp'])
        if self.start_timestamp > 0:
            edges_data = edges_data[edges_data.Timestamp >= self.start_timestamp]
        if self.verb:
...
#         test_edge_ts = torch.from_numpy(pd.concat([test_edges_data['Timestamp'], test_edges_data['Timestamp']]).to_numpy(dtype=np.int32))
        ...
        neg_eid_nums = dgl.to_simple(self.train_graph).number_of_edges()
        test_neg_eid_nums = dgl.to_simple(self.test_graph).number_of_edges()
        ...
        # DBLP neg edges
        with open("./data/DBLP/node2vec_walks_generated_adj.pkl", 'rb') as f:
            walks_dataset = pkl.load(f)
        walks_graph = dgl.graph((walks_dataset.row_ind, walks_dataset.col_ind), num_nodes=self.num_nodes)
        # self.train_neg_graph = dgl.graph(dgl.sampling.global_uniform_negative_sampling(self.graph, neg_eid_nums), num_nodes=self.num_nodes)
        # self.test_neg_graph = dgl.graph(dgl.sampling.global_uniform_negative_sampling(self.graph, test_neg_eid_nums), num_nodes=self.num_nodes)
        self.train_neg_graph = dgl.graph(dgl.sampling.global_uniform_negative_sampling(walks_graph, 20*neg_eid_nums), num_nodes=self.num_nodes)
        self.test_neg_graph = dgl.graph(dgl.sampling.global_uniform_negative_sampling(walks_graph, test_neg_eid_nums), num_nodes=self.num_nodes)
        ...
        if self.verb:
            ...
            print("train edges: {}, test edges: {}, train neg edges: {}, test neg edges: {}"\
                .format(self.train_graph.number_of_edges(), self.test_graph.number_of_edges(), 
                        self.train_neg_graph.number_of_edges(), self.test_neg_graph.number_of_edges()))
