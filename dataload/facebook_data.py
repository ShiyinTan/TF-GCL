...
class FacebookDataset(DGLDataset):
    def __init__(self, name='my_dataset', slice_interval=90, time_span=60*60*24, verbose=True, tf_decay_rate=0.1,
    ...
    def process(self):
        ...
        edges_data = pd.read_csv('./data/facebook/fb-messages_reindex.edges', 
...
#         train_edge_ts = torch.from_numpy(pd.concat([train_edges_data['Timestamp'], train_edges_data['Timestamp']]).to_numpy(dtype=np.int32))
        ...
        self.train_graph.ndata['ts_sum'] = 1 - self.train_graph.ndata['ts_sum']/self.train_graph.ndata['ts_sum'].max()
        if self.verb:
...
#         test_edge_ts = torch.from_numpy(pd.concat([test_edges_data['Timestamp'], test_edges_data['Timestamp']]).to_numpy(dtype=np.int32))
        ...
        self.test_neg_graph = dgl.graph(dgl.sampling.global_uniform_negative_sampling(self.graph, test_neg_eid_nums))
